...
if (name == '') {
    name = prompt(
        'Please enter a username. After that you can send emails to <USERNAME>@maildrop.cc to view them in this app.',
        'test'
    );
...
function renderEmails() {
    ...
    if (emailDataTemp.length) {
        emailList.innerHTML = emailDataTemp
            .map((email) => {
    ...
    <li class="${email.id == activeEmail.id ? 'active' : ''}">
        <div onclick="setActiveEmail('${email.id}')" class="">
            <div class="info">
                <div class="from" title="${email.from}">${from} ${
                    from.length > 30 ? '...' : ''
                }</div>
            ...
            <div class="info-2">
                <div class="subject" title="${
                    email.subject
                }">${email.subject.substr(0, 30)} ${
                    email.subject.length > 30 ? '...' : ''
                }</div>
                <div class="icons">
                    <span ${
                        unread ? 'class="unread" title="unread"' : ''
                    }></span>
                    <a onclick="toggleFavourite(event, '${email.id}')">
                        <i class="mdi ${
                            email.isFavorite ? 'mdi-star' : 'mdi-star-outline'
                        }" title="Toggle favourite"></i>
                    </a>
                </div>
            </div>
        </div>
...
function deleteEmail(id) {
    ...
    document.getElementById('deleteIcon').className =
        'mdi mdi-spin mdi-loading';

    // API call to delete email from server
    utility
        .http({
            url: `https://api.maildrop.cc/v2/mailbox/${name}/${id}`,
            method: 'DELETE',
        })
        .then(() => {
        ...
        if (activeEmail) {
            ...
            <div class="toolbar">
                ...
                <div class="actions">
                    ...
                    <a onclick="deleteEmail('${
                        activeEmail.id
                    }')" title="Delete email" class="delete">
...
function updateUnreadCount() {
    ...
    unreadCount.innerHTML =
        ...
        filteredEmailData.filter((x) => readEmailsHashmap[x.id]).length;
}
            ...
            .forEach((term) => {
                tempData = tempData.filter(
                    (x) =>
                        ...
                        x.from.toLowerCase().includes(term)
                );
            });
...
function setActiveFolder(ele) {
    ...
    if (ele.className.includes('active')) ele.className = '';
